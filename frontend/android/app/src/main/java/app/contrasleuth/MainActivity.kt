...
class MainActivity : BridgeActivity() {
    ...
    override fun onRequestPermissionsResult(requestCode: Int,
        ...
        when (requestCode) {
            locationRequestCode -> {
                if (grantResults.isEmpty() || grantResults[0] != PackageManager.PERMISSION_GRANTED || (grantResults.size > 1 && grantResults[1] != PackageManager.PERMISSION_GRANTED)) {
                    this.finishAffinity()
                } else {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ActivityCompat.requestPermissions(this, arrayOf(
                    Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_BACKGROUND_LOCATION), locationRequestCode)
        } else {
            ActivityCompat.requestPermissions(this, arrayOf(
